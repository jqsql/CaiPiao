    ...
    public void startDown() {
        ...
        if (targetApkFile.exists()) {
            ...
            if (info != null) {
                ...
                if (String.valueOf(latestVersion.getVersionNnumber()).equals(versionCode)) {
                    installApk();
                    return;
                }
            }
        ...
        cur.close();
        DownloadManager.Request task = new DownloadManager.Request(Uri.parse(latestVersion.getUpdateAddress()));
        //定制Notification的样式
        String title = "最新版本:" + latestVersion.getVersionNnumber();
        task.setTitle(title);
