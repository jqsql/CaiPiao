...
public class FiveStarPlayFragment extends BaseFragment {
    ...
    private List<NumberPlayBean> mList4 = new ArrayList<>();
    private List<NumberPlayBean> mList5 = new ArrayList<>();
    ...
    private int mType;//7:五星直选;8:五星通选
    ...
    private void initData() {
        ...
        mList4 = ToolUtils.depCopy(mList1);
        mList5 = ToolUtils.depCopy(mList1);
        mPlayAdapter01 = new StarPlayAdapter(getActivity(), mList5, 5);
        mPlayAdapter02 = new StarPlayAdapter(getActivity(), mList4, 4);
        mPlayAdapter03 = new StarPlayAdapter(getActivity(), mList3, 3);
        mPlayAdapter04 = new StarPlayAdapter(getActivity(), mList2, 2);
        mPlayAdapter05 = new StarPlayAdapter(getActivity(), mList1, 1);
        ...
        if (bundle != null) {
            ...
            mType = bundle.getInt("Types");
            mCheckList = (SparseArray<List<Integer>>) bundle.getSerializable("Content");
            isModification = bundle.getBoolean("isModification");
            ID = bundle.getLong("ID");
            if (mCheckList != null) {
                mPlayAdapter01.setCheckedList(mCheckList.get(5));
                mPlayAdapter02.setCheckedList(mCheckList.get(4));
                mPlayAdapter03.setCheckedList(mCheckList.get(3));
                mPlayAdapter04.setCheckedList(mCheckList.get(2));
                mPlayAdapter05.setCheckedList(mCheckList.get(1));
                setData();
            } else {
    ...
    private void setLostNumber(List<List<String>> mLostList) {
        if (mLostList != null && mLostList.size() >= 5) {
            ...
            if (list1 != null && list1.size() == mList1.size()) {
                ...
                mPlayAdapter05.notifyDataSetChanged();
            }
            ...
            if (list2 != null && list2.size() == mList2.size()) {
                ...
                mPlayAdapter04.notifyDataSetChanged();
            }
            ...
            List<String> list4 = mLostList.get(1);
            if (list4 != null && list4.size() == mList4.size()) {
                for (int i = 0; i < mList4.size(); i++) {
                    mList4.get(i).setLostNumber(list4.get(i));
                }
                mPlayAdapter02.notifyDataSetChanged();
            }
            List<String> list5 = mLostList.get(0);
            if (list5 != null && list5.size() == mList5.size()) {
                for (int i = 0; i < mList5.size(); i++) {
                    mList5.get(i).setLostNumber(list5.get(i));
                }
    ...
    private void clear() {
        ...
        mPlayAdapter04.setCheckedList(null);
        mPlayAdapter05.setCheckedList(null);
    }
}
